...
import { LayoutDashboard, Store, Package, ShoppingCart, LogOut, ShieldCheck, UserCircle, Cpu, Globe, Zap, Network, Map, Mountain } from "lucide-react";
...
function AdminSidebar() {
  ...
  const menuItems = [
    { title: "Regional Status", icon: LayoutDashboard, href: "/admin" },
    { title: "Branch Registry", icon: Store, href: "/admin/stores" },
    { title: "SKU Catalog", icon: Package, href: "/admin/inventory" },
    { title: "Transit Control", icon: ShoppingCart, href: "/admin/orders" },
  ];
    ...
    <Sidebar collapsible="icon" className="bg-[#050608] border-r border-white/5 text-slate-400">
      <SidebarHeader className="h-28 flex items-center px-8 border-b border-white/5 bg-[#050608]">
        <Link href="/admin" onClick={handleLinkClick} className="flex items-center gap-4 font-bold text-white overflow-hidden">
          <div className="bg-emerald-500 p-3 rounded-2xl shadow-[0_0_25px_rgba(16,185,129,0.3)] shrink-0">
            <Mountain className="h-6 w-6 text-white" />
          </div>
          <div className="flex flex-col group-data-[collapsible=icon]:hidden">
            <span className="text-lg tracking-tighter font-black italic">NE CONNECT</span>
            <span className="text-[9px] text-emerald-400 font-black uppercase tracking-[0.3em]">Regional Root</span>
          </div>
        </Link>
      </SidebarHeader>
      ...
      <SidebarContent className="px-4 pt-10 bg-[#050608]">
        <SidebarGroup>
          <SidebarGroupLabel className="px-4 text-[9px] uppercase font-black tracking-[0.4em] text-slate-600 mb-6">Network Protocols</SidebarGroupLabel>
          <SidebarGroupContent>
                ...
                <SidebarMenuItem key={item.title}>
                  <SidebarMenuButton 
                    ...
                    className={cn(
                      ...
                      pathname === item.href 
                        ? "bg-emerald-600 text-white font-black shadow-[0_12px_24px_rgba(16,185,129,0.2)]" 
        ...
        {user && (
          <div className="group-data-[collapsible=icon]:hidden p-5 bg-white/[0.02] border border-white/5 rounded-3xl space-y-4">
            <div className="flex items-center justify-between text-[9px] font-black text-slate-500 uppercase tracking-widest">
              <span className="flex items-center gap-2"><Network className="h-3 w-3" /> Admin ID</span>
              <div className="h-1.5 w-1.5 rounded-full bg-emerald-500 shadow-[0_0_10px_#10b981] animate-pulse" />
            </div>
            <div className="text-[10px] font-mono break-all text-emerald-400 bg-black/40 p-4 rounded-2xl border border-white/5 select-all leading-tight">
              {user.uid}
            </div>
          </div>
        )}
          ...
          <SidebarMenuItem>
            <SidebarMenuButton asChild className="h-14 rounded-2xl hover:bg-rose-500/10 hover:text-rose-500 transition-colors">
              <Link href="/" onClick={handleLinkClick}>
                ...
                <span className="text-xs font-black uppercase tracking-widest">Terminal Exit</span>
              </Link>
            </SidebarMenuButton>
          </SidebarMenuItem>
        </SidebarMenu>
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  return (
    <SidebarProvider defaultOpen={true}>
      <div className="flex min-h-svh w-full bg-[#02040a] text-slate-200 font-body">
        <AdminSidebar />
        <SidebarInset className="flex flex-col min-w-0 bg-transparent border-l border-white/5">
          <header className="sticky top-0 z-30 flex h-28 shrink-0 items-center gap-8 border-b border-white/5 bg-[#02040a]/80 backdrop-blur-3xl px-12">
            ...
            <div className="flex flex-col">
              <h2 className="text-xl font-black text-white tracking-tighter uppercase italic">Operations Control</h2>
              <div className="flex items-center gap-3">
                <Map className="h-3 w-3 text-emerald-400 animate-pulse" />
                <span className="text-[10px] text-slate-500 font-black uppercase tracking-[0.3em]">Active Hub: NE-Region-Alpha</span>
              </div>
            </div>
            
            <div className="ml-auto flex items-center gap-8">
              <div className="hidden lg:flex items-center gap-4 px-6 py-3 bg-emerald-500/5 border border-emerald-500/20 rounded-2xl">
                <div className="h-2 w-2 rounded-full bg-emerald-500 shadow-[0_0_10px_#10b981]" />
                <span className="text-[10px] font-black text-emerald-400 uppercase tracking-widest">Regional Latency: 42ms</span>
              </div>
