...
import { User, LogOut, History, PlusCircle, Leaf } from "lucide-react";
      ...
      <div className="container flex h-20 items-center justify-between px-8 mx-auto max-w-7xl">
        <Link href="/dashboard" className="flex items-center gap-3 group">
          <div className="relative">
            <div className="relative bg-primary p-2 rounded-xl text-white group-hover:rotate-12 transition-all">
              <Leaf className="h-5 w-5" />
            </div>
          </div>
          <div className="flex flex-col">
            <span className="font-black text-lg text-slate-900 uppercase tracking-tighter leading-none">Branch Hub</span>
            <span className="text-[8px] font-black text-primary uppercase tracking-[0.3em] mt-1">Regional Logistics</span>
          </div>
        </Link>
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-3">
            {isAdmin && (
              <Link href="/admin">
                <Button className="h-9 px-4 rounded-lg bg-slate-900 text-white font-black uppercase text-[9px] tracking-widest hover:bg-primary transition-all shadow-sm">
                  Command
                </Button>
              </Link>
            )}
            <Link href="/dashboard/profile">
              <Button variant="outline" size="icon" className="h-9 w-9 rounded-lg border-slate-200 text-slate-500 hover:bg-white hover:text-primary transition-all shadow-sm">
